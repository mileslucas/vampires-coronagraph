...
The RF skin depth (``\delta_e``) is the distance which an electric field's amplitude reduces by a factor of $e$. It is a material property of the electrical resistivity (``\rho``) and magnetic permeability (``\mu``). The skin depth can be calculated from [[1]](https://en.wikipedia.org/wiki/Skin_effect#Formula)

```math
\delta_e(\omega) = \sqrt{\frac{2\rho}{\omega \mu}}
```

we translate it to light frequency with ``\omega = 2\pi f`` and wavelength with ``f=c_0/\lambda``. We also use the magnetic susceptibility to describe the permeability ``\mu = \mu_0(1 + \chi_m)``

```math
\delta_e(\lambda) = \sqrt{\frac{\rho}{\pi \mu_0 (1 + \chi_m)}\frac{\lambda}{c_0}}
```

The skin depth is defined for the electric field, but for our optics we are concerned with the intensity ``I=|E|^2``, so we need to translate to the *penetration depth* (which is the inverse of the absorption coefficient, ``\alpha``).

```math
\delta_p(\lambda) \equiv \frac{1}{\alpha(\lambda)} = \frac{\delta_e(\lambda)}{2}
```
...
# ╔═╡ 0f249dcf-a75e-4eb7-bcf3-1f13c2a7cd75
skin_depth(λ; ρ, χm) = sqrt(ρ * λ / (π * μ0 * (1 + χm) * c0))

# ╔═╡ 3e67833e-ceff-42a1-afb1-7670dabd2fc0
penetration_depth(λ; kwargs...) = 0.5 * skin_depth(λ; kwargs...)

# ╔═╡ 4788fc3c-b833-4d3a-9aae-8168cd0de80f
thickness(attenuation, λ; kwargs...) = -penetration_depth(λ; kwargs...) * log(attenuation)
...
md"""
Getting all my values from [here](https://www.engineeringtoolbox.com/resistivity-conductivity-d_418.html)
"""

# ╔═╡ c8f5ed5e-50c5-4b9a-98f8-997ac5841b5a
Au = (ρ = 2.24e-8u"Ω*m", χm = -1.42e-7)

# ╔═╡ e2f9a995-fc18-4c5f-97bd-b7fea0283d5d
penetration_depth(700u"nm"; Au...) |> u"nm"
...
Cr = (ρ = 13e-8u"Ω*m", χm = 3.5e-6)

# ╔═╡ 5b3858e6-abea-49cc-978b-af239f26188e
penetration_depth(700u"nm"; Cr...) |> u"nm"
...
git-tree-sha1 = "72eacdc97b205de0b090ebc3100744b2538b6973"
