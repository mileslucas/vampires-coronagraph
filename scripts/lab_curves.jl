...
curves = @showprogress map(noisemaps) do (psf_noise, occ_psf_noise)
    ...
    psf_contrast = @. Metrics.calculate_contrast(sigma, psf_curve / starphot)
    occ_psf_contrast = @. Metrics.calculate_contrast(sigma, occ_psf_curve / starphot)
...
@info "plotting"
fig, axes = pro.subplots(refwidth="6.5in", refheight="3.66in")
axes.plot(radius[psfm], curves[1][2][psfm], c="k", label="PSF")

for (i, datum) in enumerate(curves)
    axes.plot(radius[m[:, i]], datum[3][m[:, i]], c=colors[i]["color"], label=names[i])
    axes.axvline(iwas[i:i] * 1e-3, c=colors[i]["color"], ls=":")
end

axes.legend(loc="best", ncols=1)
axes.format(
    xlabel="separation [\"]",
