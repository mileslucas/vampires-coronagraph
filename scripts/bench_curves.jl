...
datadir(args...) = joinpath("/Volumes/mlucas SSD1/vampires-coronagraph-data/bench_20220526", args...)
...
inds = map(ang -> get_cutout_inds(axes(calib_frames[1]), sep, ang; width=30, center), angles)
...
noisemaps = @showprogress "calculating noise maps" map(img -> detectionmap(noise, img, fwhm), centered_frames)

sigma = 5 # 5σ contrast curves
curves = @showprogress "calculating contrast curves" map(noisemaps, satresults) do noisemap, (phot, _)
